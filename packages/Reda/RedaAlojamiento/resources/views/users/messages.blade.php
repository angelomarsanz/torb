		...
		@foreach ($messages as $message)
            ...
            <div class="message-list-reda {{ $esMio ? 'me' : '' }} d-flex align-items-start">
                <div class="reda-avatar-container flex-shrink-0">
                    <img src="{{ $message->custom_sender_foto ?? reda_get_profile_src($message->sender) }}" class="rounded-circle reda-avatar-30 shadow-sm border" title="{{ $message->custom_sender_name ?? ($message->sender->first_name ?? 'Usuario') }}">
                </div>
